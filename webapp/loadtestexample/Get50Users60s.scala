...
class Get50Users60s extends Simulation {
  ...
  private val uri2 = "http://ocsp.pki.goog/gts1c3"

  private val scn = scenario("Get50Users60s")
    .exec(
      http("request_0")
        .get("/")
        .headers(headers_0)
        .resources(
          ...
          http("request_4")
            .post(uri2)
            .headers(headers_4)
            .body(RawFileBody("get50users60s/0004_request.dat"))
            .check(bodyBytes.is(RawFileBody("get50users60s/0004_response.dat")))
        )
    )
    .pause(1)
    .exec(
      http("request_5")
        .post(uri2)
        .headers(headers_4)
        .body(RawFileBody("get50users60s/0005_request.dat"))
        .check(bodyBytes.is(RawFileBody("get50users60s/0005_response.dat")))
    )
    .pause(1)
    .exec(
      http("request_6")
        .get("/ProfileViewer?code=438ae86774ab548ada1c16aeb03dd54f&state=43fc910284774afdb14f57a263546051")
        .headers(headers_6)
        .check(bodyBytes.is(RawFileBody("get50users60s/0006_response.html")))
        .resources(
          http("request_7")
            .get("/static/media/icono.09c0a0411923c3de503a.png")
            .headers(headers_2)
